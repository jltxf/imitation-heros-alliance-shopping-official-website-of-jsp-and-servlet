	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (myFile != null && !myFile.isMissing()) {
				...
				System.out.println(request.getContextPath() + "/image/hero/"
						+ fileName);
				// 将上传的文件保存到指定路径处
				myFile.saveAs("/image/hero/" + fileName,
			...
			hf.setName(req.getParameter("namex"));
			hf.setType(Integer.parseInt(req.getParameter("type")));
			hf.setPicture(fileName);
			HeroDAO heroDAO = new HeroDAO();
			heroDAO.update(hf, req.getParameter("namexx"));
