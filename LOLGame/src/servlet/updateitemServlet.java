	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (myFile != null && !myFile.isMissing()) {
				...
				System.out.println(request.getContextPath() + "/image/item/"
						+ fileName);
				// 将上传的文件保存到指定路径处
				myFile.saveAs("/image/item/" + fileName,
			...
			it.setName(req.getParameter("namex"));
			it.setPicture(fileName);
			ItemDAO itemDAO = new ItemDAO();
			itemDAO.update(it, req.getParameter("namexx"));
